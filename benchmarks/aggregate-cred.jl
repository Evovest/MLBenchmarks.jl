...
for data_name in data_names
    ...
    path = joinpath(@__DIR__, "..", "results", "$data_name", "evotrees-credV1A.csv")
    ...
    cred = stack(cred; variable_name="metric", value_name="credV1A")
    leftjoin!(ref, cred; on="metric")

    path = joinpath(@__DIR__, "..", "results", "$data_name", "evotrees-credV2A.csv")
    cred = CSV.read(path, DataFrame)
    transform!(groupby(cred, :model_type), metric => ordinalrank => :rank)
    subset!(groupby(cred, :model_type), :rank => (x -> x .== minimum(x)))
    select!(cred, metrics .=> (x -> round.(x, sigdigits=3)) .=> metrics)
    cred = stack(cred; variable_name="metric", value_name="credV2A")
    leftjoin!(ref, cred; on="metric")

    if data_name ∈ ["boston/raw", "year/raw", "msrank/raw"]
