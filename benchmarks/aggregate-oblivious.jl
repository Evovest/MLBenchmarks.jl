...
for data_name in data_names
    ...
    path = joinpath(@__DIR__, "..", "results", "$data_name", "evotrees-oblivious.csv")
    obli = CSV.read(path, DataFrame)
    transform!(groupby(obli, :model_type), metric => ordinalrank => :rank)
    subset!(groupby(obli, :model_type), :rank => (x -> x .== minimum(x)))
    select!(obli, metrics .=> (x -> round.(x, sigdigits=3)) .=> metrics)
    obli = stack(obli; variable_name="metric", value_name="oblivious")
    leftjoin!(ref, obli; on="metric")
