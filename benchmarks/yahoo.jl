...
results = Dict{Symbol,Any}[]
for (i, hyper) in enumerate(hyper_list)
    train_time = @elapsed bst = XGBoost.xgboost(dtrain, watchlist=OrderedDict(["eval" => deval]); nthread=nthreads, hyper...)
...
results = Dict{Symbol,Any}[]
for (i, hyper) in enumerate(hyper_list)
    model = CatBoost.CatBoostRegressor(; thread_count=nthreads, hyper...)
