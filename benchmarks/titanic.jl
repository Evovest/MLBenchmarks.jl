...
# NeuroTrees
################################
hyper_list = MLBenchmarks.get_hyper_neurotrees(hyper_size; data.loss, data.metric, nrounds=200, early_stopping_rounds=2, lr=2e-2, k=[4, 8, 16], ntrees=[16, 32, 64], depth=[3, 4, 5], hidden_size=[8, 16, 32], init_scale=0.1, batchsize=1024)
results_df = run_experiment(:NeuroTabModels, data, hyper_list; data.metrics)
