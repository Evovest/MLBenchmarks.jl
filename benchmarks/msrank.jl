...
hyper_list = MLBenchmarks.get_hyper_neurotrees(; loss=:mse, metric=:mse, nrounds=200, early_stopping_rounds=2, lr=1e-3, ntrees=[32, 64, 128, 256], stack_size=[1], depth=[3, 4, 5], hidden_size=[8, 16, 32, 64], init_scale=0.01, batchsize)
...
hyper_list = MLBenchmarks.get_hyper_lgbm(objective="mse", metric=["mse"], num_iterations=5000, early_stopping_round=10, learning_rate=0.05, num_leaves=[32, 128, 512, 2048], bagging_fraction=[0.4, 0.6, 0.8, 1.0], feature_fraction=[0.4, 0.6, 0.8, 1.0], lambda_l2=[0, 1, 10])
hyper_list = sample(hyper_list, hyper_size, replace=false)
...
hyper_list = MLBenchmarks.get_hyper_catboost(objective="RMSE", eval_metric="RMSE", iterations=5000, early_stopping_rounds=10, learning_rate=0.05, max_depth=4:2:10, subsample=[0.4, 0.6, 0.8, 1.0], rsm=[0.4, 0.6, 0.8, 1.0], reg_lambda=[0, 1, 10])
hyper_list = sample(hyper_list, hyper_size, replace=false)
