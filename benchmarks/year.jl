...
# TabM
################################
hyper_list = MLBenchmarks.get_hyper_tabm(hyper_size; data.loss, data.metric, device=:gpu, nrounds=200, early_stopping_rounds=2, lr=1e-3, arch_type=:tabm, k=[8, 16, 32], d_block=[32, 64, 128], n_blocks=2:3, dropout=0.1, batchsize=1024, embedding_type=:piecewise, d_embedding=[8, 16], nbins=[16, 32])
...
# NeuroTrees
################################
hyper_list = MLBenchmarks.get_hyper_neurotrees(hyper_size; data.loss, data.metric, device=:gpu, nrounds=200, early_stopping_rounds=2, lr=1e-3, k=[8], ntrees=[32, 64, 128], stack_size=[1], depth=[3, 4, 5], hidden_size=[8, 16, 32], init_scale=0.1, batchsize=1024, embedding_type=:piecewise, d_embedding=[8, 16], nbins=[16, 32])
results_df = run_experiment(:NeuroTabModels, data, hyper_list; data.metrics)
