...
# NeuroTrees
################################
hyper_list = MLBenchmarks.get_hyper_neurotrees(hyper_size; data.loss, data.metric, device=:gpu, nrounds=200, early_stopping_rounds=2, lr=1e-3, k=[4, 8, 16], ntrees=[32, 64, 128], stack_size=[1], depth=[3, 4, 5], hidden_size=[8, 16, 32], init_scale=1.0, batchsize=1024)
results_df = run_experiment(:NeuroTabModels, data, hyper_list; data.metrics)
